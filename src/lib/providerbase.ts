import { ProviderInterface, TaskInterface, ClientInfo, SessionConfig, ResourceTimingEntry } from "../@types"
import assign from "../util/assign"
import prefixKeys from "../util/prefixKeys"
import { FetchConfig, FetchResult, FetchBeaconData } from "./fetch"
...
export default abstract class ProviderBase implements ProviderInterface {
    ...
    abstract makeClientInfoPromise(task: TaskInterface): Promise<ClientInfo>
    abstract fetchSessionConfig(): Promise<SessionConfig>
    abstract expandTasks(): void

    getCurrentTestId(): string | undefined {
        if (this.sessionConfig && this.sessionConfig.test) {
            return this.sessionConfig.test.id
        }
    }

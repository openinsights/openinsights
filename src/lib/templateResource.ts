...
type ResourceReducer =
    | ((resource: string, config: SessionConfig) => string)
...
function replaceTestId(resource: string, config: SessionConfig): string {
    let testId = 'unknown'
    if (config.test) {
        testId = config.test.id
    }
    return resource.replace(/<%TEST_ID%>/gi, testId)
}
